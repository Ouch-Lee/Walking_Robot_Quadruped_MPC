...
void CalculateAMat(const Vector3d& rpy, MatrixXd* a_mat_ptr) { // CT方程
    ...
    angular_velocity_to_rpy_rate << cos_yaw / cos_pitch, sin_yaw / cos_pitch, 0,
        -sin_yaw, cos_yaw, 0, cos_yaw* tan_pitch, sin_yaw* tan_pitch, 1; // 旋转矩阵
...
void CalculateExponentials(const MatrixXd& a_mat, const MatrixXd& b_mat,
    ...
    MatrixXd* a_exp_ptr, MatrixXd* b_exp_ptr) { // DT 离散状态
    const int state_dim = ConvexMpc::kStateDim; //  kStateDim = 13; = 6 dof pose + 6 dof velocity + 1 gravity.
    MatrixXd& ab_mat = *ab_mat_ptr;
    ab_mat.block<state_dim, state_dim>(0, 0) = a_mat * timestep; // A * delta t
    const int action_dim = b_mat.cols(); // 获取B矩阵列数
    ab_mat.block(0, state_dim, state_dim, action_dim) = b_mat * timestep; // ab = [A B]

    // This temporary is inevitable.
    MatrixXd ab_exp = ab_mat.exp();
    *a_exp_ptr = ab_exp.block<state_dim, state_dim>(0, 0);
    *b_exp_ptr = ab_exp.block(0, state_dim, state_dim, action_dim); // 通过指数求得离散状态下的A B矩阵，而不是A*delta_t + I
}
...
void CalculateQpMats(const MatrixXd& a_exp, const MatrixXd& b_exp,
    ...
    MatrixXd* b_qp_ptr, MatrixXd* p_mat_ptr) {  // 引入MPC，求得Aqp Bqp
    ...
    for (int i = 1; i < horizon; ++i) {
        a_qp.block<state_dim, state_dim>(i * state_dim, 0) =
            a_exp * a_qp.block<state_dim, state_dim>((i - 1) * state_dim, 0); // [A; A*A; A*A*A; ...]
    }
...
void UpdateConstraintsMatrix(std::vector<double>& friction_coeff,
    ...
    const int constraint_dim = ConvexMpc::kConstraintDim; // constraint_dim = 5
    MatrixXd& constraint = *constraint_ptr;
    for (int i = 0; i < horizon * num_legs; ++i) { // 预测数目*腿的数目
        constraint.block<constraint_dim, k3Dim>(i * constraint_dim, i * k3Dim) // <5,3>(5*i, 3*i)
            << -1,
...
void CalculateConstraintBounds(const MatrixXd& contact_state, double fz_max,
    ...
    VectorXd* constraint_ub_ptr) {                                                  // 二次规划的力约束情况（上下限）
    const int constraint_dim = ConvexMpc::kConstraintDim; // constraint_dim = 5

    const int num_legs = contact_state.cols(); // 腿数
    ...
    for (int i = 0; i < horizon; ++i) {
        for (int j = 0; j < num_legs; ++j) {
            ...
            constraint_lb(row + 4) = fz_min * contact_state(i, j); // 如果接触地面，则contact_state(i, j) = 1，反之则是0

            const double friction_ub =
                (friction_coeff + 1) * fz_max * contact_state(i, j); // 不等式部分只需要考虑最大值即可，因为C*u有正有负
            ...
            constraint_ub(row + 4) = fz_max * contact_state(i, j); // 如果接触地面，则contact_state(i, j) = 1，反之则是0
        }
    }
}
...
double EstimateCoMHeightSimple(
    ...
    for (int i = 0; i < foot_contact_states.size(); ++i) {
        if (foot_contact_states[i]) { // 如果触地
            com_height += foot_positions_world(i, z_dim); // 质心高度和腿部抬起高度 （这地方其实不太懂，为什么足部的z可以估算质心的z，而且还是接触地面的足部)
            legs_in_contact += 1;
        }
    }
...
MatrixXd AsBlockDiagonalMat(const std::vector<double>& qp_weights,
    ...
    const MatrixXd qp_weights_mat =
        qp_weights_vec.replicate(planning_horizon, 1).asDiagonal(); // 求权重矩阵L，是对角矩阵，即(x-x_ref)T * L * (x-x_ref)
...
std::vector<double> ConvexMpc::ComputeContactForces(
    ...
    const Quaterniond com_rotation =
        ...
        AngleAxisd(com_roll_pitch_yaw[2], Vector3d::UnitZ()); // 从旋转矩阵得到四元数 详见ref:ceres-solver/include/ceres/rotation.h
    ...
    for (int i = 0; i < num_legs_; ++i) {
        foot_positions_world_.row(i) = com_rotation * foot_positions_base_.row(i); // 质心坐标系旋转矩阵*质心坐标系下足部坐标 = 世界坐标系下的足部坐标
    }
    ...
    const double com_z =
        com_position.size() == k3Dim // == 3? 是，则com_position[2]当做质心z坐标，不是就用足部简单估计质心高度
        ? com_position[2]
        : EstimateCoMHeightSimple(foot_positions_world_, foot_contact_states); // 根据足部坐标求得质心高度 （这地方其实不太懂，为什么足部的z可以估算质心的z)
    ...
    // Prepare the current and desired state vectors of length kStateDim *  状态的一个汇总，开始准备MPC
    // planning_horizon.
    ...
    CalculateAMat(rpy, &a_mat_); // 计算A

    rotation_ = ConvertRpyToRot(rpy); // 将欧拉角转换为旋转矩阵
    const Matrix3d inv_inertia_world =
        rotation_ * inv_inertia_ * rotation_.transpose(); // 将当前坐标的空间惯量转换为世界坐标系的空间惯量，详情可见潘阳的高等机构动力学

    CalculateBMat(inv_mass_, inv_inertia_world, foot_positions_world_, &b_mat_); // 计算B矩阵
    ...
    CalculateConstraintBounds(contact_states_, mass_ * kGravity * kMaxScale, // 约束矩阵
        mass_ * kGravity * kMinScale,
        foot_friction_coeffs[0], planning_horizon_,
        &constraint_lb_, &constraint_ub_);
    
    //  后面都是优化过程，可以参考之前的注释以及MIT的文献Convex Model-Predictive Control那篇来阅读 //
